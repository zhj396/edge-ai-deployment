...
# ---------------------------------------------------------------------------
def _assemble_batch(results: List[dict], device: str = "cpu", fp16: bool = False) -> Dict:
...
def preprocess_frames(
    ...
    # caller enables save= (the server does not).
    results = [
        _preprocess_bgr(f, imgsz=imgsz_t, original=original, path=f"<frame:{i}>")
