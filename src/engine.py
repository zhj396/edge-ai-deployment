...
class YOLOv8Engine:
    ...
    def _run_batch(
        ...
        if not isinstance(outputs, (np.ndarray, torch.Tensor, list, tuple)):
            raise TypeError(
        ...
        if save:
            for j, dets in enumerate(batch_dets):
                ...
                # preprocess_frames stores synthetic "<frame_N>" paths. Mirrors
    ...
    def infer_frames(
        ...
        if save:
            ...
            # keeps the request path viable on a read-only container FS.
            output_dir.mkdir(parents=True, exist_ok=True)
