...
The control arm for the staged long-tail experiment in train_staged/ (same subset,
same backbone, no long-tail countermeasures) — and, per the unified evaluation in
docs/TRAINING.md §6, the producer of the deployed checkpoint. Ran on Kaggle Tesla T4 x2.
...
release, and silent version drift invalidates the pinned-environment
guarantee — the class-weight hook this pipeline relies on only exists in
ultralytics >= 8.4 (docs/TRAINING.md §5).
"""
...
SRC_DATA = Path("/kaggle/input/datasets/<KAGGLE_USERNAME>/coco-subset/coco_subset_12cls")
