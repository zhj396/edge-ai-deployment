...
- **Training** (`train/train_baseline.py`, `train/train_staged/`) runs on **Kaggle
  ...
  ultralytics`, which floats to the latest release and previously caused version
  drift. The native `class_weights` hook the injection relies on only exists in
  ultralytics >= 8.4). Two Kaggle dataset uploads back the runs: the **baseline** script
  (`train/train_baseline.py`) reads `coco-12cls`, the **staged** script
  (`train/train_staged/config.yaml`) reads `coco-subset` — two uploads of the same
  `coco_subset_12cls/` inner directory under different slugs. The hardcoded
  `/kaggle/...` paths record that environment.
...
diagnosis: moderate long tail (max/min ≈ 8.03) — motivates the staged schedule
   │
   ├─ train/train_baseline.py        single-stage SGD fine-tune (the baseline / ablation)
   │
   └─ train/train_staged/            three-stage (A/B/C) AdamW training with custom
                                     class weights — the FINAL checkpoint producer
   ▼
yolov8s.pt (12 cls) ──→ main.py export / quantize / benchmark / consistency
...
- The vectorized Hamming scan preserves brute-force semantics exactly (same greedy
  ...
  ~10⁸ Python-level Hamming comparisons a per-pair `ImageHash` loop costs at the 10k
  scale.
- The ResNet-18 embedding similarity check is O(N²) numpy over a growing list
  (documented in the code) — measured acceptable at the 10k scale; a
...
Role in the project: the **ablation reference** for the staged run — same subset, same
backbone, no long-tail countermeasures.

## 5. Staged long-tail training (`train_staged/`) — final checkpoint
...
Each cell comes from the corresponding run's `results.csv` / wandb output — numbers
produced by any other code or config do not belong in this table.
...
The headline numbers for interviews are the **per-class deltas on the tail classes
(backpack, bicycle)** across the three rows — that is the point of the long-tail design.
...
> `models/yolov8s.pt`. Swapping in a different checkpoint requires re-running the
> whole downstream pipeline (`main.py export → quantize → benchmark → consistency`),
> or the two halves of the repo describe different models.

## 7. Handoff to the deployment repo

The staged training's checkpoint is staged as `models/yolov8s.pt` (see the
deployment-side README §Installation) and consumed by `python main.py export` — from
there the deployment pipeline takes over: ONNX FP32 → INT8 QDQ (calibration images
chosen by `src/sampler.py` from the same subset's val split) → 7-backend
consistency/benchmark. See the checkpoint-coupling warning in §6 before swapping in
different weights.
