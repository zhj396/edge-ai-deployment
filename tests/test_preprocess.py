...
def test_preprocess_frames_batch_contract():
    ...
    # Synthetic names must stay Windows-filename-safe: the save path does
    # Path(...).name on these, and ":" is illegal in Windows filenames.
    assert data["paths"] == ["<frame_0>", "<frame_1>"]


def test_preprocess_frames_matches_file_backed_path(tmp_path):
    """Array path must be pixel-identical to the file path for the same image."""
    frame = _synthetic_bgr()
    path = tmp_path / "parity.png"
    cv2.imwrite(str(path), frame)  # PNG is lossless -> file decodes back to `frame`
    decoded = cv2.imread(str(path))

    from_file = preprocess_single(str(path), imgsz=640)
